...
const ProductListingPage = () => {
  ...
  useEffect(() => {
    ...
    if (filters.categorys?.length) {
      filtered = filtered.filter((p) =>
        filters.categorys.includes(p.category.value)
      );
    }
    ...
    if (filters.gender?.length) {
      filtered = filtered.filter((p) =>
        filters.gender.includes(p.gender.value)
      );
    }
    ...
    if (sortOption === "menor-preco") {
      ...
      filtered.sort((a, b) => {
        const score = (p) => {
          ...
          if (normalize(filters.condition) === normalize(p.condition.value))
            s += 1;
  ...
  const updateFilter = (filterKey, value) => {
    ...
    if (filterKey === "categorys") {
      ...
      brand = brand.includes(value)
        ? brand.filter((m) => m !== value)
        : [...brand, value];
      searchParams.delete("brand");
      brand.forEach((m) => searchParams.append("brand", m));
    } else if (filterKey === "gender") {
      gender = gender.includes(value)
  ...
  const buildFilterText = () => {
    ...
    filters.brand.forEach((brand) => {
      const brandObj = products.filter.brand.find(
        (m) => m.value === normalize(brand)
      );
      parts.push(`Marca: ${brandObj ? brandObj.name : brand}`);
    });
    filters.gender.forEach((gen) => {
      const genderObj = products.filter.gender.find(
        (g) => g.value === normalize(gen)
      );
      parts.push(`Gênero: ${genderObj ? genderObj.name : gen}`);
    });
    if (filters.condition) {
      const conditionObj = products.filter.condition.find(
        (e) => e.value === normalize(filters.condition)
      );
      parts.push(
        `Condição: ${conditionObj ? conditionObj.name : filters.condition}`
      );
    }
  ...
  const renderActiveFilters = () => {
    ...
    if (filters.condition)
      activeFilters.push({
        key: "condition",
        label: `Condição: ${filters.condition}`,
